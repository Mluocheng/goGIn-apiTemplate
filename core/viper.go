...
import (
	"flag"
	"fmt"
	"goGIn-apiTemplate/core/internal"
	"goGIn-apiTemplate/global"
	"os"
...
func InitializeViper(path ...string) *viper.Viper {
	...
	if len(path) == 0 {
		...
		fmt.Println("gin.DebugMode      ", gin.DebugMode)
	...
	fmt.Println("os.Getenv(internal.ConfigEnv):", os.Getenv(internal.ConfigEnv))
	fmt.Println("config:", config)
	vip := viper.New()
